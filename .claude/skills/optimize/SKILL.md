...
### No-cook only (stabilizers)
**All stabilizers are cold-process (no-cook).** NEVER use corn starch — it requires cooking to 90°C+ to gelatinize. Use cold-acting stabilizers instead: xanthan, guar, CMC, inulin, whey protein. These provide body and thickening without heat. **Exception:** Cooking for *flavor development* (e.g. caramelizing milk, blooming cocoa, reducing fruit) is fine — the liquid is cooled before mixing with cold-process stabilizers.
...
6. **Scoopulator URL format:** `https://scoopulator.app/calc?ingredients=slug1:weight1,slug2:weight2,...&target=TARGET` where TARGET is one of the **slugs** from the Target Profiles section below. **Type parameter:** For non-ice-cream profiles, append `&type=TYPE` using the Type column from the "Other Profiles" table (e.g. `&type=sorbet`, `&type=gelato`). Ice cream profiles omit `&type=` entirely. For `freeform`, use whichever type matches the actual recipe. Choose the correct TARGET profile for what the recipe actually is — don't game the metrics by picking whichever target happens to score more greens. When a recipe fits multiple profiles, present both Scoopulator URLs (one per candidate target) and let the user choose. If a recipe doesn't clearly fit any profile, inform the user and let them decide which target to validate against (or whether to use `freeform`).
    **Evaporation parameter:** For recipes that reduce/cook a liquid ingredient (e.g. caramelizing milk), append `&evp=FRACTION` to the URL where FRACTION is the decimal fraction of water lost (e.g. `&evp=0.22` for 22% evaporation). This tells Scoopulator to concentrate the solids while removing only water — much more accurate than entering the reduced weight of the raw ingredient. Enter the **original (pre-reduction) weight** of the ingredient. Example: 700g milk reduced by 22% → enter `milk:700` with `&evp=0.22`, and Scoopulator computes the correct 546g final weight with concentrated solids.
...
8. **Show every Scoopulator fetch** as a one-line color-coded summary — every iteration, not just the final one. Never silently skip a fetch. Format: `PAC 23.8 | Serv. -9.4°C 🔴 | Sweet 9.5% 🔴 | Sugars 14.1% 🟡 | Solids 27% 🔴 | Fat 2.6% 🔴 | Stab 0.42% 🔴 | Emul 0% 🔴`
   **Ignore these metrics** in the summary line — do NOT include them: Milk Fat, MSNF. These are always red/unusable for our low-fat no-sugar style (no cream → no milk fat; low dairy → no MSNF). They are noise.
   **Evaluate these 7 metrics** (the ones that predict real-world quality for our recipes): Serving Temperature, Relative Sweetness, Sugars, Total Solids, Total Fat, Stabilizers, Emulsifiers.
9. **Always paste the Scoopulator URL** after every validation summary. The user should never have to ask for it. Paste the **raw URL** on its own line right after the one-line summary — no markdown link syntax, no bullet point. Just the bare URL so it's easy to copy-paste from the console.
10. **Target profile selection:** Choose the profile based on **what the recipe is** by ingredient character — NOT by which profile produces more greens.
    - **Decide first:** Look at the dominant ingredients and their *purpose* — ask "what type of frozen dessert is this trying to be?" Consider why each ingredient is there, not just that it's present. Yogurt as the star flavor → froyo. Yogurt as a fat-trimmer in a cheesecake base → still ice cream. Fruit as the base → sorbet. Milk-only base with sweeteners → low-fat or nonfat ice cream. Cream cheese as the star → ice cream (cheesecake style).
    - **Then optimize toward that profile's targets.** If Sugars is red because the froyo profile expects 15-17% and we're at 9%, that's a signal our recipe is structurally different — accept it or adjust, but don't switch to general-ice-cream just because it has a wider Sugars range.
    - **Profile can change if ingredients fundamentally shift** during optimization (e.g., adding cream moves a recipe from low-fat to standard). But never switch profiles just to chase greens.
    - Present your profile reasoning so the user can override.

...
> 5. **Scoopulator result display (EVERY fetch):** After **every single** Scoopulator fetch — including intermediate optimization loops, not just the final result — immediately show the user a **minimal one-line** color-coded summary. Format: `PAC value | Metric value 🟢/🟡/🔴 | ...`. Example:
>    ```

...
> 3. **Scoopulator URL format:** `https://scoopulator.app/calc?ingredients=slug1:weight1,slug2:weight2,...&type=TYPE` where TYPE is one of: `ice_cream`, `frozen_yogurt`, `gelato`, `sherbet`, `sorbet`, `ganache`, `other`.
> 4. **Ingredient slugs:** Look up in `rest/scoopulator_ingredients_index.txt` (format: `name:slug`). You can also use the user's custom ingredients.
